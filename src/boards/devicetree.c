...
static void map(struct ExpansionBoard *board)
{
    ...
    vmm_map(mmu_virt2phys((uintptr_t)board->rom_file), board->map_base, board->rom_size, 0x77c, 0);
    vmm_map(mmu_virt2phys((uintptr_t)fdt_base), board->map_base + board->rom_size, (fdt_base->totalsize + 4095) & ~4095, 0x77c, 0);
}
